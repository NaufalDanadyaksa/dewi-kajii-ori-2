...
<div class="container">
    <form action='{{ route('wisata.update', $wisata->id) }}' method='POST' enctype="multipart/form-data">
        ...
        <div class="form-group">
            <input type="text" class="form-control text-white" name='name' id="name" style="background-color: #68687D; text-decoration: none; opacity: 1; box-shadow: none; border: none; border-radius: 6px; margin-bottom:10px;" value="{{ $wisata->name }}" required>
        </div>
        <div class="form-group">
            <input type="number" class="form-control text-white" name="price" id="price" style="background-color: #68687D; text-decoration: none; opacity: 1; box-shadow: none; border: none; border-radius: 6px; margin-bottom:10px;" value="{{ $wisata->price }}" required>
        </div>
        <div class="form-group">
            @foreach ($wisata->content as $content)
            <input type="text" class="form-control mt-2 text-white" id="contents1" name="contents[]" style="background-color: #68687D; text-decoration: none; opacity: 1; box-shadow: none; border: none; border-radius: 6px; margin-bottom:10px;" placeholder="nama paket" value="{{ $content->content }}">
            @endforeach
        </div>
        ...
        <div class="form-group">
            ...
            <input type="file" name='new_image[]' id="image" multiple class="form-control rounded-pill text-white" style="display:none;">
