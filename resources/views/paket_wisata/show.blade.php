...
<div class="container mt-5">
    <div class="row justify-content-center">
        <div class="col-md-5 order-md-1 order-2">
            <div id="carouselDetailwisata" class="carousel slide" data-ride="carousel">
                <div class="carousel-inner">
                    @foreach($wisata->images as $index => $image)
                    <div class="carousel-item {{$index == 0 ? 'active' : ''}}">
                        <img src="{{ asset('/posts/paket_wisata/'.$image->url) }}" class="img-card-homestay d-block w-100" alt="..." style="height: 12rem; border-radius: 10px;">
                    </div>
                    @endforeach
                </div>
                <ol class="carousel-indicators">
                    @foreach($wisata->images as $index => $image)
                    <li data-target="#carouselDetailwisata" data-slide-to="{{$index}}" class="{{$index == 0 ? 'active' : ''}}"></li>
                    @endforeach
                </ol>
                <a class="carousel-control-prev" href="#carouselDetailwisata" role="button" data-slide="prev">
                    <span class="carousel-control-prev-icon" aria-hidden="true"><i class="ti ti-chevron-left"></i></span>
                    <span class="sr-only">Previous</span>
                </a>
                <a class="carousel-control-next" href="#carouselDetailwisata" role="button" data-slide="next">
        ...
        <div class="col-md-5 order-md-2 order-1 mt-3 mt-md-0">
            <div class="container rounded p-4" style="background-color: #68687D">
                <div class="text">
                    <h3 class="text-white">{{$wisata->name}}</h3>
                    <p class="text-white">{{$wisata->price}}</p>
