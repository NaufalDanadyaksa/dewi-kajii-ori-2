...
<form action='{{ route('atraksi.update', $atraksi->id) }}' method='post' enctype="multipart/form-data">
    ...
    <div class="my-3 p-3 bg-body rounded shadow-sm">
        <a href='{{ url('atraksi') }}' class="btn btn-secondary"><< Kembali</a>
        
        <div class="mb-3 row">
            <label for="nama" class="col-sm-2 col-form-label">Name</label>
            <div class="col-sm-10">
                <input type="text" class="form-control" name='name' value="{{ $atraksi->name }}" id="name">
            </div>
        </div>
        <div class="mb-3 row">
            <label for="description" class="col-sm-2 col-form-label">Description</label>
            <div class="col-sm-10">
                <textarea name="description" id="description" class="form-control">{{$atraksi->description}}</textarea>
            </div>
        </div>
